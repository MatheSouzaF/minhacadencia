...
import { type UserCategory, DEFAULT_CATEGORIES, PALETTE_COLORS } from '@/types'
...
export function OnboardingModal() {
  ...
  const [selected, setSelected] = useState<UserCategory[]>(DEFAULT_CATEGORIES)
          ...
          {step === 'welcome' ? (
            ...
            >
              ...
              <div className="overflow-y-auto flex-1 px-6 py-4 space-y-2">
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        style={{ backgroundColor: cat.color.startsWith('var') ? undefined : cat.color, background: cat.color }}
                      />
                      <span className="text-lg">{cat.emoji}</span>
                      <span className="text-sm text-white font-medium flex-1 text-left">{cat.name}</span>
                      {isSelected && <Check className="w-4 h-4 text-zinc-400 shrink-0" />}
                    </button>
                  )
                })}
                ...
                {selected
                  .filter((c) => !DEFAULT_CATEGORIES.find((d) => d.id === c.id))
                  .map((cat) => (
                    <div
                      key={cat.id}
                      className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-zinc-600 bg-zinc-800"
                    >
              ...
              <div className="px-6 pb-6 pt-4 border-t border-zinc-800">
                <button
                  ...
                  className="w-full bg-white text-zinc-900 font-medium py-2.5 rounded-xl hover:bg-zinc-100 transition-colors disabled:opacity-40"
                >
